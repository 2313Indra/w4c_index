// Navigation Toggle - Priority Function
(function() {
...
function calculateGovernanceRealm() {
    // Monitoring and Alert System (single question)
    const alertSystemScore = document.getElementById('alert-system').value === 'yes' ? 1 : 0;
    document.getElementById('alert-system-score').textContent = alertSystemScore;

    // Interdepartmental Coordination (single question)
    const coordinationScore = document.getElementById('coordination').value === 'yes' ? 1 : 0;
    document.getElementById('coordination-score').textContent = coordinationScore;
    
    // Citizen Engagement (two questions)
    const citizenPoliciesScore = document.getElementById('citizen-policies').value === 'yes';
    const citizenParticipationScore = document.getElementById('citizen-participation').value === 'yes';
    const citizenEngagementScore = (citizenPoliciesScore || citizenParticipationScore) ? 1 : 0;
    document.getElementById('citizen-engagement-score').textContent = citizenEngagementScore;
    
    // Mediation Platforms (two questions)
    const conflictMechanismsScore = document.getElementById('conflict-mechanisms').value === 'yes';
    const collaborativeSolutionsScore = document.getElementById('collaborative-solutions').value === 'yes';
    const mediationScore = (conflictMechanismsScore || collaborativeSolutionsScore) ? 1 : 0;
    document.getElementById('mediation-score').textContent = mediationScore;
    
    // Subsidies (two questions)
    const waterSubsidiesScore = document.getElementById('water-subsidies').value === 'yes';
    const researchFundingScore = document.getElementById('research-funding').value === 'yes';
    const subsidiesScore = (waterSubsidiesScore || researchFundingScore) ? 1 : 0;
    document.getElementById('subsidies-score').textContent = subsidiesScore;
    
    // Connection with Water (single question)
    const connectionScore = document.getElementById('water-workshops').value === 'yes' ? 1 : 0;
    document.getElementById('connection-score').textContent = connectionScore;
    
    // Adequate Maintenance (four questions)
    const maintenanceFundingScore = document.getElementById('maintenance-funding').value === 'yes';
    const longTermPlanningScore = document.getElementById('long-term-planning').value === 'yes';
    const maintenanceGuidelinesScore = document.getElementById('maintenance-guidelines').value === 'yes';
    const assetDatabasesScore = document.getElementById('asset-databases').value === 'yes';
    const maintenanceScore = (maintenanceFundingScore || longTermPlanningScore || 
                             maintenanceGuidelinesScore || assetDatabasesScore) ? 1 : 0;
    document.getElementById('maintenance-score').textContent = maintenanceScore;
    
    // NGOs and SHGs (two questions)
    const ngosShgsScore = document.getElementById('ngos-shgs').value === 'yes';
    const ngoCollaborationScore = document.getElementById('ngo-collaboration').value === 'yes';
    const ngoScore = (ngosShgsScore || ngoCollaborationScore) ? 1 : 0;
    document.getElementById('ngo-score').textContent = ngoScore;
...
function calculateWaterSensitivityLevel() {
    // Check if all Technology realm inputs are filled
    const techInputs = [
        'resilience-score', 'failure-rate', 'integration-score', 'drainage-index',
        'harvesting-index', 'recycling-index', 'water-import-rate', 'water-availability',
        'leakage-percentage', 'sanitation-state', 'treatment-capacity'
    ];
    
    // Check if all Environment realm inputs are filled
    const envInputs = [
        'aqua-health-index', 'green-blue-ratio', 'water-circularity'
    ];
    
    // Check if all Society realm inputs are filled
    const societyInputs = [
        'literacy-score', 'vulnerability-index', 'satisfaction-score', 'accessibility-index'
    ];
    
    // Check if all Governance realm inputs are filled
    const governanceInputs = [
        'alert-system', 'coordination', 'citizen-policies', 'citizen-participation',
        'conflict-mechanisms', 'collaborative-solutions', 'water-subsidies', 'research-funding',
        'water-workshops', 'maintenance-funding', 'long-term-planning', 'maintenance-guidelines',
        'asset-databases', 'ngos-shgs', 'ngo-collaboration'
    ];

    // Function to check if an input has a valid value
    const isInputFilled = (id) => {
        const element = document.getElementById(id);
        if (!element) return false;
        const value = element.tagName === 'SELECT' ? element.value : element.textContent;
        return value !== '' && value !== '-' && !isNaN(parseFloat(value));
    };

    // Check if all inputs in each realm are filled
    const allTechFilled = techInputs.every(isInputFilled);
    const allEnvFilled = envInputs.every(isInputFilled);
    const allSocietyFilled = societyInputs.every(isInputFilled);
    const allGovernanceFilled = governanceInputs.every(id => {
        const element = document.getElementById(id);
        return element && element.value !== '';
    });

    // If any realm is not completely filled, show "Please fill all indicators"
    if (!allTechFilled || !allEnvFilled || !allSocietyFilled || !allGovernanceFilled) {
        document.getElementById('water-sensitivity-level').textContent = "Please fill all indicators";
        return;
    }

    // If all inputs are filled, proceed with calculation
    ...
    if (minRealmScore <= 1.5 && governanceScore < 0.2) {
        ...
        sensitivityLevel = "Scores do not align with sensitivity levels";
    }
